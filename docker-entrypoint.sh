...
if [ "$IAM_ROLE" == "none" ]; then
  ...
  /usr/bin/s3fs ${S3_BUCKET} ${MOUNT_POINT} -o url=${URL},nosuid,nonempty,nodev,allow_other,default_acl=${S3_ACL},retries=5
else
  echo 'IAM_ROLE is set - using it to mount S3'
  /usr/bin/s3fs ${S3_BUCKET} ${MOUNT_POINT} -o iam_role=${IAM_ROLE},url=${URL},nosuid,nonempty,nodev,allow_other,default_acl=${S3_ACL},retries=5
fi
